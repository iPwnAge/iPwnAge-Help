...
public class HelpCommand implements CommandExecutor {
	...
	public void displayHelp(Player p){
		...
		misc.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("Learn more about other various commands").create()));
	...
	private void tp(Player p){
		...
		if(p.hasPermission(normal) || p.hasPermission(commoner) || p.hasPermission(dcommoner)){
			...
			TextComponent normie = new TextComponent("As a new player, you can only use /tpa, and /warp. You'll gain more commands as you rank-up. Click on a command, below, to learn how to use it.");

			normie.setColor(ChatColor.GRAY);

			tpa.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("Teleport to other players, with their permission.").create()));
			tpa.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/tpa"));

			warp.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("Warp to public, player-created warps around the server!").create()));
		...
		if(p.hasPermission(vet) || p.hasPermission(dvet) || p.hasPermission(senior) || p.hasPermission(dsenior) || p.hasPermission(elder) || p.hasPermission(delder) || p.hasPermission(arch) || p.hasPermission(trib)){
			...
			TextComponent dontSpam = new TextComponent("However, spamming /tphere on someone, and tp'ing to staff without their explicit permission is a punishable offense!");
